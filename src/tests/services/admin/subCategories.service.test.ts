...
describe('Sub-Categories Service Testing', () => {
  ...
  describe('Create Sub-Category', () => {
    describe('Create sub-category', () => {
      ...
      beforeAll(async () => {
        subCategoryService.subcategory.create = jest
          .fn()
          .mockResolvedValue(createdSubCategory);
        ret = await subCategoryService.create({} as any);
      });
      test('should return the created sub-category id', () => {
        expect(ret).toBe(createdSubCategory);
      });
    });
  });

  describe('Update Sub-Category by id', () => {
    describe('Update sub-category', () => {
      const updatedSubCategory = {
        id: '12345'
      };
      beforeAll(async () => {
        subCategoryService.subcategory.update = jest
          .fn()
          .mockResolvedValue(updatedSubCategory);
        ret = await subCategoryService.updateById(1, {});
      });
      test('should return the updated sub-category id', () => {
        expect(ret).toBe(updatedSubCategory);
      });
    });
  });

  describe('Delete Sub-Category by id', () => {
    describe('Delete sub-category', () => {
      beforeAll(async () => {
        subCategoryService.subcategory.delete = jest
          .fn()
          .mockResolvedValue(null);
      });
      test('should delete the sub-category', () => {
        expect(
          async (): Promise<void> => await subCategoryService.deleteById(1)
        ).not.toThrow();
      });
    });
  });
